...
export async function POST(request) {
  try {
    ...
    // Check which form is being submitted based on the fields
    if (body.bloodGroup !== undefined) {
      ...
      // Format the data for the Blood Donation form
      values = [
        [
          body.name,
          body.age,
      ...
      range = "BloodDonations!A:J"; // Separate sheet for blood donations
    } else {
      // MHL Form
      if (!body.name || !body.phoneNumber) {
        return NextResponse.json(
      ...
      // Format the data for the MHL form
      values = [
        [
          body.name,
          body.phoneNumber,
      ...
      range = "MHL!A:F"; // Separate sheet for MHL submissions
    }
