...
const validateServerMobileNumber = (mobileNumber) => {
    ...
    return /^[6-9]\d{9}$/.test(String(mobileNumber));
};
...
export async function POST(request) {
  try {
    ...
    // --- Health Package Form ---
    if (body.employeeId !== undefined && body.healthPackage !== undefined) {
...
//     // --- Health Package Form (New) ---
//     // Differentiated by unique fields like employeeId and healthPackage
//     if (body.employeeId !== undefined && body.healthPackage !== undefined) {
//       // Server-side validation for Health Package form
//       if (!body.employeeId || !body.name || !body.email || !body.mobileNo || !body.gender || !body.age || !body.healthPackage || !body.address || !body.pincode) {
//         return NextResponse.json({ message: "All fields are required for health package registration." }, { status: 400 });
//       }
...
//       }
//       if (!validateServerMobileNumber(body.mobileNo)) {
//         return NextResponse.json({ message: "Invalid 10-digit mobile number." }, { status: 400 });
//       }
//       if (!validatePincode(body.pincode)) {
//         return NextResponse.json({ message: "Invalid 6-digit pincode." }, { status: 400 });
//       }
...
//           submissionDate
//       ]];
//       range = "HealthPackage!A:J"; // Sheet named 'HealthPackage', columns A to J
...
//       return NextResponse.json({ message: "Data submitted successfully", response: response.data }, { status: 200 });

//     } catch (googleError) {
//       console.error("Google Sheets API Error:", googleError);
//       if (googleError.message?.includes("invalid_grant")) {
//         authClient = null; sheetsClient = null;
//         return NextResponse.json({ message: "Authentication error. Please try again or contact support." }, { status: 401 });
//       }
//       if (googleError.message?.includes("permission")) {
//         return NextResponse.json({ message: "Permission denied. Contact support." }, { status: 403 });
//       }
